<?php $page_title = 'Update Phone - SFCI - Journal for Critical Indicents';
    ...
    if (isset($_GET["p"])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($errors)) {
                ...
                mysqli_query($dbc, "Call spUpdatePhoneNumber($PhoneID,$PhoneType,'$PhoneNumber',$Primary);");
            ...
            $PhoneInfo = mysqli_query($dbc, "Call spGetUserPhoneInfo($PhoneID);");
            complete_procedure($dbc);
            
            if (mysqli_num_rows($PhoneInfo) == 1) {
                $PhoneInfo = mysqli_fetch_array($PhoneInfo, MYSQLI_ASSOC);
                
                if ($PhoneInfo["UserID"] == $UserID) {
                    $PhoneType = $PhoneInfo["PhoneTypeID"];
                    $PhoneNumber = $PhoneInfo["PhoneNumber"];
                    $Primary = $PhoneInfo["PrimaryPhone"];
                } else {
                    //Redirect user back to the account settings page
                    redirect_user('account_settings.php');  
                }
        ...
        <div class="col s7">
            ...
            <div class="box_guest author_alt">
                ...
                <form method="post">
                    ...
                    <button class="author buttonform" type="submit">Update Phone Number</button>
                </form>
            </div>
        </div>
