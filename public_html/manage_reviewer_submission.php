<?php
$page_title = 'Manage Critical Incident Review Files';
...
if (isset($_GET['rid']) && is_numeric($_GET['rid'])) {
    $revID = mysqli_real_escape_string($dbc, $_GET["rid"]);
}
else {
...
if (!$error && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$error && isset($_FILES)) {
        $q_subfiles = "CALL spSubmissionGetFilesList($revID);";
        $subfiles = array();
    ...
    if (!$error && !$incomplete) {
        $q_update = "CALL spUpdateSubmissionStatus($revID, 3);";
...
if (!$error) {
    ...
    $q_submission = "CALL spSubmissionGetInfo($revID);";
    if ($r_submission = mysqli_query($dbc, $q_submission)) {
        ...
        if ($r_subfiles = mysqli_query($dbc, $q_subfiles)) {
            while ($row_subfiles = mysqli_fetch_array($r_subfiles)) {
                ...
                array_push($rev_filelist, $row_subfiles);
            }
            complete_procedure($dbc);
        }
        else {
...
if (!$error && !$success) {
    ...
    if (count($rev_filelist) > 0) {
        echo "        <div class=\"editor roundcorner\">\r\n";
        echo "            <h3 class=\"title\">Manage Critical Incident Review Files: $sub_title </h3>\r\n";
        echo "        </div>\r\n";
        echo "        <div style=\"padding-left:50px;\" class=\"box_guest editor_alt\" id=\"registration-form\">\r\n";
        echo "            <form class=\"submitform\" method=\"post\" action=\"manage_author_submission.php?rid=$revID\" enctype=\"multipart/form-data\">\r\n";
        foreach ($rev_filelist as $filetype) {
...
if ($error || $incomplete) {
    ...
    echo "        </p>\r\n";
    if (isset($revID)) {
        echo "        <p><a href=\"manage_reviewer_submission.php?rid=$revID\">Retry Review Cleaning</a></p>";
    }
    else {
        echo "        <p><a href=\"view_reviewer_submissions.php\">Return to List of Reviewer Submissions</a></p>";
    }
}
echo "    </div>\r\n";
